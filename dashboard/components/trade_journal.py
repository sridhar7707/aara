...
def render_trade_journal() -> str:
    ...
    if stats:
        ...
        for i, s in enumerate(stats):
            td = TD if i < len(stats) - 1 else TD0
            wr_color = GAIN if s["win_rate"] >= 55 else (LOSS if s["win_rate"] < 40 else TEXT2)
            avg_color = GAIN if s["avg_pct"] > 0 else LOSS
            stat_rows += (
                f"<tr>"
                f"<td {td} style='{_CELL1}'>{_reason_label(s['exit_reason'])}</td>"
                f"<td {td} style='{_NUM}'>{s['n']}</td>"
                f"<td {td} style='{_NUM}color:{wr_color};'>{s['win_rate']:.0f}%</td>"
                f"<td {td} style='{_NUM}color:{avg_color};'>{'+' if s['avg_pct']>=0 else ''}{s['avg_pct']:.1f}%</td>"
                f"</tr>"
            )
        pattern_table = (
            f"<table style='width:100%;border-collapse:collapse;'>"
            f"<thead><tr>"
            f"<th {TH} style='text-align:left;'>Exit Type</th>"
            f"<th {TH} style='text-align:right;'>Trades</th>"
            f"<th {TH} style='text-align:right;'>Win Rate</th>"
            f"<th {TH} style='text-align:right;'>Avg Return</th>"
    ...
    if not rows:
        ...
        for i, r in enumerate(rows):
            ...
            tbody += (
                f"<tr>"
                f"<td {td} style='{_CELL1}'>{r.get('symbol','—')} {status_tag}</td>"
                f"<td {td} style='{_CELL}'>{_fmt_date(r.get('created_at'))}</td>"
                f"<td {td} style='{_CELL}'>{conf_s}</td>"
                f"<td {td} style='{_CELL}'>{_reason_label(r.get('exit_reason'))}</td>"
                f"<td {td} style='{_NUM}color:{oc};'>{_outcome_str(pct)}</td>"
                f"<td {td} style='{_CELL}font-style:italic;color:{TEXT3};max-width:300px;"
                f"white-space:normal;'>{lesson}</td>"
                f"</tr>"
            )
        table = (
            f"<table style='width:100%;border-collapse:collapse;'>"
            ...
            f"<th {TH} style='text-align:left;'>Entry Date</th>"
            f"<th {TH} style='text-align:left;'>Confidence</th>"
            f"<th {TH} style='text-align:left;'>Exit Type</th>"
