"""Tests for applications.trading_intelligence.adapters.trust_ledger_snapshot.

Covers the ADR-064 Section 2.1 / Section 10 contract for the read-only,
consumer-only ``trust_ledger.db`` HuggingFace dataset snapshot pull, plus
the ADR-061 health contract:
  ...
  - HF_DB_REPO_ID (the ADR-055 identity, reused) is still a required
    configuration guard (NOT_CONFIGURED);
  ...
  - the copy target is the product-owned ``.runtime`` path, never
    ``./trust_ledger.db`` and never ``data/trust_ledger.db``;
  - every failure mode fails closed with value=None and a classified
    IntegrationHealth; there is no fallback to any other database;
  - the module performs no HF write/upload/commit and imports nothing
    under bot/ ledger/ scheduler/ dashboard/ database/ sentinel_engine/.

``hf_hub_download`` is always faked -- no network.
"""
import ast
import fnmatch
import inspect
...
from applications.trading_intelligence.adapters import trust_ledger_snapshot
from applications.trading_intelligence.adapters.trust_ledger_snapshot import (
    fetch_trust_ledger_db_snapshot,
)

_MODULE_SRC = inspect.getsource(trust_ledger_snapshot)
...
def runtime_dir(tmp_path, monkeypatch):
    rt = tmp_path / ".runtime"
    snap = rt / "trust_ledger_snapshot.db"
    monkeypatch.setattr(trust_ledger_snapshot, "_RUNTIME_DIR", rt)
    monkeypatch.setattr(trust_ledger_snapshot, "_SNAPSHOT_PATH", snap)
    return snap
...
def _fake_download_factory(tmp_path, *, contents=_SQLITE_BYTES):
    ...
    def _fake(**kwargs):
        calls.append(kwargs)
        cache_file = tmp_path / "hf_cache" / "trust_ledger.db"
...
def test_not_in_space_is_not_configured_without_download(monkeypatch, runtime_dir):
    ...
    _assert_failed(fetch_trust_ledger_db_snapshot(), IntegrationStatus.NOT_CONFIGURED)
...
def test_in_space_success_is_healthy_with_the_runtime_path(
    ...
    result = fetch_trust_ledger_db_snapshot()
    ...
    assert fake.calls[0]["filename"] == "trust_ledger.db"
...
def test_in_space_empty_token_still_attempts_public_repo(
    ...
    assert result.health.status is IntegrationStatus.HEALTHY
    assert len(fake.calls) == 1
    assert fake.calls[0]["token"] is None
...
def test_copy_target_is_runtime_path_never_cwd_or_data_dir(
    tmp_path, monkeypatch, runtime_dir, in_space, repo_configured
):
    ...
    resolved = pathlib.Path(fetch_trust_ledger_db_snapshot().value).resolve()

    assert resolved != (tmp_path / "trust_ledger.db").resolve()
    assert not (tmp_path / "trust_ledger.db").exists()
    assert resolved.name != "trust_ledger.db"
    assert resolved.parent.name == ".runtime"


def test_default_snapshot_path_is_inside_the_product_runtime_area():
    path = pathlib.Path(trust_ledger_snapshot._snapshot_path())
    assert path.name == "trust_ledger_snapshot.db"
    assert path.name != "trust_ledger.db"
    parts = path.parts
    assert parts[-2] == ".runtime"
    assert parts[-3] == "trading_intelligence"
    assert parts[-4] == "applications"
    # never the bot's working ledger path
    assert "data" not in parts[-3:]


def test_timeout_constant_is_twenty_seconds():
    assert trust_ledger_snapshot._FETCH_TIMEOUT_SECONDS == 20
...
def test_generic_download_exception_fails_closed(
    ...
    assert not runtime_dir.exists()  # no fallback file written
...
def test_404_entry_not_found_is_api_error(
    ...
    def _not_found(**kwargs):
        raise Exception("404 Client Error. Entry Not Found for url: .../trust_ledger.db")

    monkeypatch.setattr("huggingface_hub.hf_hub_download", _not_found)
    _assert_failed(fetch_trust_ledger_db_snapshot(), IntegrationStatus.API_ERROR)
...
def test_auth_rejected_download_is_auth_failed(
    ...
    _assert_failed(fetch_trust_ledger_db_snapshot(), IntegrationStatus.AUTH_FAILED)
...
def test_copy_error_is_api_error(
    ...
    monkeypatch.setattr(trust_ledger_snapshot.shutil, "copy", _copy_boom)
    _assert_failed(fetch_trust_ledger_db_snapshot(), IntegrationStatus.API_ERROR)
...
def test_download_timeout_is_unavailable_and_does_not_block(
    ...
    monkeypatch.setattr(trust_ledger_snapshot, "_FETCH_TIMEOUT_SECONDS", 0.3)
...
def test_abandoned_slow_worker_cannot_leave_a_partial_snapshot(
    ...
    monkeypatch.setattr(trust_ledger_snapshot, "_FETCH_TIMEOUT_SECONDS", 0.3)
    release = threading.Event()
    source = tmp_path / "hf_cache" / "trust_ledger.db"
    ...
    leftovers = [
        p.name for p in runtime_dir.parent.glob(".trust_ledger_snapshot.*.part")
    ]
    assert leftovers == []
...
def test_module_imports_no_protected_packages():
    tree = ast.parse(_MODULE_SRC)
    protected = {"bot", "dashboard", "scheduler", "database", "ledger", "sentinel_engine"}
    for node in ast.walk(tree):
        names = []
        if isinstance(node, ast.Import):
...
def test_module_does_not_import_the_trades_snapshot_primitive():
    """ADR-064 Section 2.1: the primitive is duplicated, not imported."""
    assert "trades_db_snapshot" not in _MODULE_SRC


def test_module_never_references_the_bot_working_ledger_path():
    assert "data/trust_ledger.db" not in _MODULE_SRC
