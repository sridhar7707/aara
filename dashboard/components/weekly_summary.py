...
def render_weekly_summary() -> str:
    ...
    for i, (sym, entry_px, hwm, opened_at) in enumerate(positions):
        ...
        pos_rows += (
            ...
            f'</td>'
            f'<td {td}>'
            f'<span style="font-size:{FONT_LABEL};color:{TEXT1};" title="{why_bought}">{why_bought}</span>'
            ...
            f'<div style="font-size:{FONT_LABEL};color:{TEXT2};margin-top:3px;" title="{hold_reason}">{hold_reason}</div>'
            f'</td>'
            f'</tr>'
        )
