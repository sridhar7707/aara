"""Tests for applications.wealth_intelligence.main.main().
...
import applications.wealth_intelligence.main as main_module
...
class _FakeInvestorWorkspaceUI:
    def __init__(self):
        self.build_calls = 0
...
def test_main_calls_build_application(monkeypatch):
    fake_ui = _FakeInvestorWorkspaceUI()
...
def test_main_imports_only_bootstrap():
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            assert module == "applications.wealth_intelligence.bootstrap", (
                f"unexpected import from {module!r}; main.py should only import "
                "applications.wealth_intelligence.bootstrap"
            )
